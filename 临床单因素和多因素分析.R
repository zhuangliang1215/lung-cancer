# ============================================================
# 完整的Cox回归分析代码（符合Lung Cancer规范）
# 功能：单因素和多因素Cox回归分析，生成SCI级森林图
# 特点：保留TX/NX/MX的临床意义，作为独立类别分析
# 输出：TIFF（500 dpi，投稿用）+ PNG（300 dpi，预览用）+ PDF（矢量图）
# ============================================================
...
library(forestplot)
library(grid)
library(dplyr)  # 用于case_when函数

# 设置工作目录（请修改为您的实际路径）
setwd("C:/Users/Administrator/Desktop/clinic")

# ============================================================
# 第一部分：数据预处理（保留TX/NX/MX的临床意义）
# ============================================================
cat("开始数据预处理...\n")
...
# ============================================================
# 变量转换（关键修改：保留TX/NX/MX作为独立类别）
# ============================================================

# 生存状态转换：Alive -> 0, Dead -> 1
clin$status <- ifelse(clin$vital_status == 'Alive', 0, 1)
cat("\n生存状态转换完成\n")
print(table(clin$vital_status, clin$status, dnn = c("原始状态", "转换后")))

# 性别转换：female -> 0, male -> 1
clin$gender_num <- ifelse(clin$gender == 'female', 0, 1)
cat("\n性别转换完成\n")
print(table(clin$gender, clin$gender_num, dnn = c("原始性别", "转换后")))

# AJCC分期转换
# Stage IA/IB -> 1, Stage IIA/IIB -> 2, Stage IIIA/IIIB -> 3, Stage IV -> 4
cat("\nAJCC分期分布:\n")
